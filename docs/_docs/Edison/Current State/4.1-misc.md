...
{% include warning.html content="This needs securing so that login with the root account is only possible from the console. We need to consider that root login over the network (`sshd`) or bluetooth (`rfcomm`) is insecure." %}

## blink-led
`systemctl start blink-led` starts  the blinking LED. `blink-led` is currently working in the non-acpi branch only.

{% include warning.html content="In the acpi enabled branch gpio have received pin names. The proper way to access these is to use `libgpiod`. There is an open issue [44](https://github.com/edison-fw/meta-intel-edison/issues/44)" %}
