...
# function ota_find_partition_alternative
# Find a partition by label
# input  u_part_lbl : partition label
# output u_part_num : partition number
setenv ota_find_partition_alternative 'if itest $ota_verbose == 1 ; then echo "part number mmc 0 ${u_part_lbl} u_part_num;"; fi; part number mmc 0 ${u_part_lbl} u_part_num;ret=$?; if itest $ret != 0 ; then setenv ota_abort_reason "part number mmc 0 ${u_part_lbl} u_part_num failed: ${ret}"; setenv ota_abort 1; fi;'
...
# function ota_use_alternative
# Check uboot commands supported syntax
# output ota_alternative : use alternative commands
setenv ota_use_alternative 'if itest $ota_verbose == 1 ; then echo "Check support for commands alternative syntax"; fi; part find mmc 0 label:boot;ret=$?; if itest $ret != 0 ; then setenv ota_alternative 1; fi;'

# function ota_update_env
# Update env
# input floadaddr : memory address destination
# input filesize  : size of image in bytes
setenv ota_update_env 'if itest $ota_verbose == 1 ; then echo "Update env"; fi; env import -c ${floadaddr} ${filesize};ret=$?; if itest $ret != 0 ; then setenv ota_abort_reason "env import -c ${floadaddr} ${filesize} failed: ${ret}"; setenv ota_abort 1; fi;'

# function ota_cleans_script
# remove from environment all variables and functions in this script
setenv ota_cleans_script 'env delete -f sha1_sum sha1_sum_ref; env delete -f u_part_blksz u_part_lbl u_part_num u_part_start u_part_sz updt_part_num; env delete -f ota_drive ota_alternative; env delete -f mod_blk num_blk filesize bytesize blksize floadaddr vb_cmd; env delete -f floadaddr; env delete -f ota_conv_sizes ota_mmc_write ota_load_image ota_compute_hash; env delete -f ota_test_image_and_partition_sizes ota_find_partition ota_find_partition_alternative ota_get_partition_attributes ota_get_partition_attributes_alternative ota_exec_verbose_cmd ota_use_alternative ota_update_env ota_cleans_script;'
...
run ota_use_alternative;

if itest $ota_alternative == 1 ; then
echo " "
echo "Using alternative syntax"
echo " "
else
echo " "
echo "Using default syntax"
echo " "
fi
...
if itest $ota_alternative == 1 ; then
run ota_find_partition_alternative ;
else
...
# Retrieve partition attribute
if itest $ota_alternative == 1 ; then
run ota_get_partition_attributes_alternative ;
else
...
if itest $ota_abort == 1 ; then run ota_cleans_script; exit ; fi;
# Start update of u-boot-envs/edison-blankcdc.bin on u-boot-env0
setenv ota_image_name u-boot-envs/edison-blankcdc.bin
setenv u_part_lbl u-boot-env0
...
if itest $ota_abort == 1 ; then run ota_cleans_script; exit ; fi;
# Start update of u-boot-envs/edison-blankcdc.bin on u-boot-env1
setenv ota_image_name u-boot-envs/edison-blankcdc.bin
setenv u_part_lbl u-boot-env1
...
# Start update of u-boot-envs/edison-blankcdc.bin in env
setenv ota_image_name u-boot-envs/edison-blankcdc.bin
